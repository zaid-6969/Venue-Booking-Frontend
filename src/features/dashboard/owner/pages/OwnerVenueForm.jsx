...
import {
  Building2, MapPin, DollarSign, Users, Plus, Trash2, ArrowLeft, Save,
  UploadCloud, Image as ImageIcon, Check, Star, X, Sparkles
} from 'lucide-react';
...
const PRESET_SAMPLE_IMAGES = [
  { label: 'Grand Banquet', url: 'https://images.unsplash.com/photo-1519167758481-83f550bb49b3?w=800' },
  { label: 'Luxury Lawn', url: 'https://images.unsplash.com/photo-1511795409834-ef04bbd61622?w=800' },
  { label: 'Royal Palace', url: 'https://images.unsplash.com/photo-1542314831-068cd1dbfeeb?w=800' },
  { label: 'Rooftop Sunset', url: 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=800' },
];
...
const OwnerVenueForm = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    description: 'Welcome to our premier event space! We provide elegant seating, ambient lighting, customizable catering options, and full air conditioning for weddings, corporate galas, and social celebrations.',
  ...
  useEffect(() => {
    if (isEdit && selectedVenue) {
      ...
      if (selectedVenue.gallery && selectedVenue.gallery.length > 0) {
        setGalleryImages(selectedVenue.gallery.map(g => ({ url: g.url, fileId: g.fileId || 'img_' + Math.random() })));
      }
    }
  ...
  const handleGalleryFileUpload = async (e) => {
    ...
    try {
      for (const file of files) {
        ...
        const url = await uploadImageFile(file);
        setGalleryImages(prev => [...prev, { url, fileId: 'img_' + Date.now() + '_' + Math.random().toString(36).substr(2, 4) }]);
      }
  ...
  const handleAddGalleryUrl = (e) => {
    ...
    setGalleryImages(prev => [...prev, { url: newGalleryUrlInput.trim(), fileId: 'url_' + Date.now() }]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (description.length < 50) {
      toast.error('Description must be at least 50 characters long (currently ' + description.length + ' chars).');
      return;
    }
    ...
    try {
      if (isEdit) {
        await dispatch(updateVenue({ id, data: payload })).unwrap();
      ...
      // Re-fetch owner venues to ensure MongoDB data is loaded into state
      dispatch(fetchMyVenues());
      navigate('/owner/venues');
    } catch (err) {
      toast.error(err.message || 'Failed to save venue listing');
    }
  };

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', maxWidth: 880, margin: '0 auto' }}>
      <div>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-8)' }}>
        ...
        <div className="card" style={{ padding: 'var(--space-6)', background: 'var(--surface-1)' }}>
          <h3 style={{ fontSize: 'var(--text-lg)', fontWeight: 700, marginBottom: 'var(--space-4)' }}>General Details</h3>
          
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
            <div>
              <label style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--text-tertiary)', display: 'block', marginBottom: 4 }}>Venue Name</label>
              <input type="text" name="name" value={formData.name} onChange={handleChange} className="input" placeholder="e.g. The Grand Palace Banquet" required />
            </div>

            <div>
              <label style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--text-tertiary)', display: 'block', marginBottom: 4 }}>Tagline</label>
              <input type="text" name="tagline" value={formData.tagline} onChange={handleChange} className="input" placeholder="e.g. Where luxury meets celebration" />
            </div>

            <div>
              <label style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--text-tertiary)', display: 'block', marginBottom: 4 }}>Category</label>
            ...
            <div>
              <label style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--text-tertiary)', display: 'block', marginBottom: 4 }}>Description</label>
              <textarea name="description" rows={5} value={formData.description} onChange={handleChange} className="input" placeholder="Provide full details about hall dimensions, lighting, ambiance, etc." required minLength={50} />
            </div>
          </div>
        </div>
        ...
        <div className="card" style={{ padding: 'var(--space-6)', background: 'var(--surface-1)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-4)' }}>
            <div>
              <h3 style={{ fontSize: 'var(--text-lg)', fontWeight: 700 }}>Venue Cover & Gallery Images</h3>
              <p style={{ fontSize: 'var(--text-xs)', color: 'var(--text-secondary)', marginTop: 2 }}>
                Upload high-resolution photos directly or enter image URLs to display on your venue listing
              </p>
            </div>
          </div>

          {/* Primary Cover / Thumbnail Upload Card */}
          <div style={{ marginBottom: 'var(--space-6)', padding: 'var(--space-5)', background: 'var(--bg-subtle)', borderRadius: 'var(--radius-xl)', border: '1px dashed var(--border-default)' }}>
            <label style={{ fontSize: 'var(--text-xs)', fontWeight: 800, color: 'var(--brand-default)', textTransform: 'uppercase', display: 'block', marginBottom: 8 }}>
              Primary Cover / Thumbnail Image
            </label>

            <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', gap: 'var(--space-6)', alignItems: 'center' }}>
              {/* Live Cover Preview */}
              <div style={{ position: 'relative', height: 140, borderRadius: 'var(--radius-lg)', overflow: 'hidden', background: '#000' }}>
                <img src={formData.coverImageUrl} alt="Cover Preview" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                <span className="badge badge-success" style={{ position: 'absolute', top: 8, left: 8, fontSize: '10px' }}>Primary Cover</span>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
                ...
                <div style={{ display: 'flex', gap: 'var(--space-3)', flexWrap: 'wrap' }}>
                  ...
                  <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-tertiary)', alignSelf: 'center' }}>or paste URL below:</div>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', flexWrap: 'wrap', marginTop: 2 }}>
                  <span style={{ fontSize: '11px', color: 'var(--text-tertiary)', fontWeight: 600 }}>Preset Samples:</span>
                  {PRESET_SAMPLE_IMAGES.map((sample, idx) => (
                    <button
                      key={idx}
                      type="button"
                      onClick={() => setFormData(prev => ({ ...prev, coverImageUrl: sample.url }))}
          ...
          <div style={{ padding: 'var(--space-5)', background: 'var(--bg-subtle)', borderRadius: 'var(--radius-xl)', border: '1px solid var(--border-subtle)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-4)', flexWrap: 'wrap', gap: 'var(--space-2)' }}>
              <div>
                <label style={{ fontSize: 'var(--text-xs)', fontWeight: 800, color: 'var(--text-primary)', textTransform: 'uppercase' }}>
                  Gallery Photos ({galleryImages.length})
                </label>
                <p style={{ fontSize: '11px', color: 'var(--text-tertiary)', marginTop: 2 }}>Add internal hall photos, dining setups, stage designs, and parking areas</p>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: 'var(--space-3)', marginBottom: 'var(--space-4)' }}>
              {galleryImages.map((img, i) => (
                <div key={i} style={{ position: 'relative', height: 100, borderRadius: 'var(--radius-lg)', overflow: 'hidden', background: '#000', border: '1px solid var(--border-subtle)' }}>
            ...
            <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
              ...
              />
              <button type="button" onClick={handleAddGalleryUrl} className="btn btn-secondary btn-sm" style={{ flexShrink: 0 }}>
                Add URL Photo
              </button>
            </div>
          </div>
        ...
        {/* Location & Pricing */}
        <div className="card" style={{ padding: 'var(--space-6)', background: 'var(--surface-1)' }}>
          <h3 style={{ fontSize: 'var(--text-lg)', fontWeight: 700, marginBottom: 'var(--space-4)' }}>Location & Pricing</h3>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)' }}>
            <div style={{ gridColumn: '1 / -1' }}>
              <label style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--text-tertiary)', display: 'block', marginBottom: 4 }}>Address</label>
              <input type="text" name="address" value={formData.address} onChange={handleChange} className="input" required />
            </div>

            <div>
              <label style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--text-tertiary)', display: 'block', marginBottom: 4 }}>City</label>
              <input type="text" name="city" value={formData.city} onChange={handleChange} className="input" required />
            </div>

            <div>
              <label style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--text-tertiary)', display: 'block', marginBottom: 4 }}>State</label>
              <input type="text" name="state" value={formData.state} onChange={handleChange} className="input" required />
            </div>

            <div>
              <label style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--text-tertiary)', display: 'block', marginBottom: 4 }}>Min Guest Capacity</label>
              <input type="number" name="minCapacity" value={formData.minCapacity} onChange={handleChange} className="input" required min={10} />
            </div>

            <div>
              <label style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--text-tertiary)', display: 'block', marginBottom: 4 }}>Max Guest Capacity</label>
              <input type="number" name="maxCapacity" value={formData.maxCapacity} onChange={handleChange} className="input" required min={10} />
            </div>

            <div style={{ gridColumn: '1 / -1' }}>
              <label style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--text-tertiary)', display: 'block', marginBottom: 4 }}>Price per Day (₹)</label>
              <input type="number" name="pricePerDay" value={formData.pricePerDay} onChange={handleChange} className="input" required min={0} />
            </div>
          </div>
        </div>
        ...
        {/* Amenities Checklist */}
        <div className="card" style={{ padding: 'var(--space-6)', background: 'var(--surface-1)' }}>
          <h3 style={{ fontSize: 'var(--text-lg)', fontWeight: 700, marginBottom: 'var(--space-4)' }}>Amenities Offered</h3>
        ...
        {/* Submit Button */}
        <button type="submit" disabled={mutateStatus === 'loading'} className="btn btn-primary btn-lg" style={{ alignSelf: 'flex-start', gap: 'var(--space-2)', fontWeight: 700 }}>
          <Save size={18} /> {mutateStatus === 'loading' ? 'Saving Listing...' : isEdit ? 'Update Listing' : 'Submit Venue Listing'}
        </button>
